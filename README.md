...
```math
\sqrt{n}(\hat{\beta}_n - \beta_0) \to_d \mathcal{N}(0, Q^{-1}\Omega Q^{-1}).
```
...
```math
\sqrt{n}(\hat{\beta}_n - \beta_0) = \hat{Q}_n^{-1}(\sqrt{n}\bar{g}_n) + R_n.
```

where $\bar{g}_n$ is the average of the scores $X_i e_i$ and $R_n$ is
zero whenever $\hat{Q}_n$ is invertible. That is `olsHat_sub_beta0` /
`normError_eq`. The scores are orthogonal, $E[Xe]=0$, because $\beta_0$
...
| WLLN | `wlln` (T1) | $\hat{Q}_n \to_p Q$ |
| CLT | `clt` (T2) | $\sqrt{n}\bar{g}_n \to_d \mathcal{N}(0,\Omega)$ |
| Slutsky | `slutsky` (T4) | $\hat{Q}_n^{-1}(\sqrt{n}\bar{g}_n)+R_n \to_d \mathcal{N}(0, Q^{-1}\Omega Q^{-1})$ |
...
turn the WLLN into $\hat{Q}_n^{-1} \to_p Q^{-1}$ and $R_n \to_p 0$
...
$E$ and moments, `Events.lean` / `Convergence.lean` for $\mathrm{Pr}$ and
